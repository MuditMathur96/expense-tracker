...
import { addReminder } from '@/db/reminer.db';
...
function AddReminder({userId}: Props) {
    const addReminderToState = useExpenseStore(state=>state.addReminder);
    const categories = useExpenseStore(state=>state.categories);
    const [loading,setLoading] = useState<boolean>(false);
    const [open,setOpen] = useState<boolean>(false);
    const [inputData,setInputData] = useState<Reminder>({
        title:"",
        price:0,
        completed:false,
        lastDay:1,
    ...
    const handleAddExpense:FormEventHandler = async(e)=>{
        e.preventDefault();
        setLoading(true);
      let res = await addReminder(inputData);
      setLoading(false);
      setOpen(false);
      if(res.error) return;
      if(res.data) addReminderToState(res.data);
      }
  ...
  return (
        ...
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
                ...
                <Button disabled={!userId}>Add Reminder</Button>
            </DialogTrigger>
            ...
            <DialogContent >
                ...
                className='space-y-4'>
                    ...
                    <div className='space-y-1'>
                        <label htmlFor='title'>Last Day</label>
                        <Input
                        onChange={handleInputChange}
                        type="number"
                        max={28}
